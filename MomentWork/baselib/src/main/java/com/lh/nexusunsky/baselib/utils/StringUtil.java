...
public class StringUtil {
    ...
    public static String getResourceString(int strId) {
        String result = "";
        if (strId > 0) {
            result = AppContext.getResource().getString(strId);
        }
        return result;
    }
    ...
    public static String getResourceStringAndFormat(int strId, Object... objs) {
        String result = "";
        if (strId > 0) {
            result = String.format(Locale.getDefault(), AppContext.getResource().getString(strId), objs);
        }
        return result;
    }
        ...
        if (!TextUtils.isEmpty(sourceString) && !TextUtils.isEmpty(keyWord) && sourceString.contains(keyWord)) {
            ...
            if (index >= 0) {
                ...
                while (index < sourceStrLen && index >= 0) {
                    spanBuilder.setSpan(new ForegroundColorSpan(color), index, index + keyWord.length(), Spannable
                            .SPAN_EXCLUSIVE_EXCLUSIVE);
                    spanBuilder.setSpan(new StyleSpan(Typeface.NORMAL), index, index + keyWord.length(), Spanned
                            .SPAN_EXCLUSIVE_EXCLUSIVE);
    ...
    public static SpannableStringBuilder highLightKeyWord(String keyWord, int defaultColor, int color,
        ...
        if (!TextUtils.isEmpty(sourceString) && !TextUtils.isEmpty(keyWord) && sourceString.contains(keyWord)) {
            spanBuilder = new SpannableStringBuilder(sourceString);
            spanBuilder.setSpan(new ForegroundColorSpan(defaultColor), 0, sourceString.length(), Spannable
                    .SPAN_EXCLUSIVE_EXCLUSIVE);
            int index = sourceString.indexOf(keyWord);
            if (index >= 0) {
                int sourceStrLen = sourceString.length();
                ...
                while (index < sourceStrLen && index >= 0) {
                    spanBuilder.setSpan(new ForegroundColorSpan(color), index, index + keyWord.length(), Spannable
                            .SPAN_EXCLUSIVE_EXCLUSIVE);
                    spanBuilder.setSpan(new StyleSpan(Typeface.NORMAL), index, index + keyWord.length(), Spannable
                            .SPAN_EXCLUSIVE_EXCLUSIVE);
                    index = sourceString.indexOf(keyWord, index + keyWord.length());
                }
            }
