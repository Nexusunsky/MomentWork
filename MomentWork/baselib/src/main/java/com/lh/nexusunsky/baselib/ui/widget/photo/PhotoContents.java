package com.lh.nexusunsky.baselib.ui.widget.photo;
...
import com.lh.nexusunsky.baselib.utils.SimpleObjectPool;
...
public class PhotoContents extends FlowLayout {
    ...
    protected LayoutParams generateDefaultMultiLayoutParams(boolean isSingle) {
        LayoutParams p;
        if (isSingle) {
            //            p = new PhotoContents.LayoutParams(ViewGroup.LayoutParams.WRAP_CONTENT, ViewGroup
    ...
    private Rect getDrawableBoundsInView(ImageView iv) {
        ...
        result.right = (int) (result.left + tDrawableRect.width() * (values[Matrix.MSCALE_X] == 0 ? 1.0f :
                values[Matrix.MSCALE_X]));
        result.bottom = (int) (result.top + tDrawableRect.height() * (values[Matrix.MSCALE_Y] == 0 ? 1.0f :
